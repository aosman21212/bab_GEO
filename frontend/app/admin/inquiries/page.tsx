...
export default function AdminInquiriesPage() {
  ...
  const statusLabel = (status: string) => {
    if (status === 'new') return t('inquiries.filterNew')
    if (status === 'read') return t('inquiries.filterRead')
    if (status === 'archived') return t('inquiries.filterArchived')
    return status
  }
  ...
  const filters: { id: StatusFilter; label: string }[] = [
    { id: 'all', label: t('inquiries.filterAll') },
    { id: 'new', label: t('inquiries.filterNew') },
    { id: 'read', label: t('inquiries.filterRead') },
    { id: 'archived', label: t('inquiries.filterArchived') },
  ]
          ...
          <table className="w-full min-w-[960px] text-start text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((item) => {
                  const preview =
                    item.project.length > 72 ? `${item.project.slice(0, 72)}…` : item.project
                  return (
        ...
        >
          ...
          >
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                <p className="text-[11px] font-bold tracking-[0.14em] text-primary">
                  {t('inquiries.modalEyebrow')}
                </p>
                <h2 id="inquiry-modal-title" className="mt-1 text-xl font-extrabold text-navy">
                  {t('inquiries.modalTitle')}
                </h2>
              </div>
              <button
                type="button"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="flex flex-col gap-1.5 text-sm font-semibold text-navy">
                {t('common.company')}
                <input className={fieldClass()} readOnly value={selected.company || '—'} />
              </label>
              <label className="flex flex-col gap-1.5 text-sm font-semibold text-navy">
                {t('common.email')}
