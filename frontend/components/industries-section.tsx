...
export function IndustriesSection({
  ...
  industries?: HomepageLocaleData['industries']
  stats?: HomepageLocaleData['stats']
}) {
  const t = useTranslations('industries')
  const ts = useTranslations('stats')
  const locale = useLocale()
  const [active, setActive] = useState<HomepageIndustryId>('food')
  const currentId = INDUSTRY_IDS.includes(active) ? active : 'food'
  const current = industries?.[currentId]
  const image = current?.image || DEFAULT_INDUSTRY_IMAGES[currentId]
            ...
            >
              <div>
                <h3 className="text-2xl font-extrabold text-navy">
                  {current?.title || t(`${currentId}.title`)}
                </h3>
                <p className="mt-4 leading-relaxed text-muted-foreground">
                  {current?.body || t(`${currentId}.body`)}
                </p>
                <a
                ...
                >
                  {industries?.bookDemo || t('bookDemo')}{' '}
              ...
              <div className="flex justify-center lg:justify-end">
                <div className="relative h-64 w-64 overflow-hidden rounded-full grayscale md:h-80 md:w-80">
                  <Image
                    src={image}
                    alt={current?.title || t(`${currentId}.title`)}
                    fill
                    sizes="320px"
